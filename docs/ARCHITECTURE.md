...
Reads have two modes. The default local mode serves read endpoints only to that shared session, exactly as before public reads existed. Setting `pixiv.public_read_enabled` switches to public-site mode: read endpoints (illustration/user/search/ranking and the SSE stream) pass a dedicated gate when `pixiv.service_refresh_tokens` holds at least one token, and each upstream read authenticates as a token drawn round-robin from that pool — never the operator session, with no fallback in either direction (a drained pool fails closed even if the operator is logged in; the validator refuses enabling the switch with an empty pool). While public mode is on, even the signed-in operator's reads upstream as pool identities: per-browser user tokens are explicitly deferred, so features that need the operator's own identity upstream (such as private bookmarks) are unavailable until that lands. Mutations never use the gate — bookmark, follow, server-side download jobs, config patches, and system actions stay operator-only, and the write gate answers on the mode alone: public mode rejects writes before consulting the session, so an operator session left in `state.json` (a leftover or migrated data root) cannot re-open them. Pool tokens that Pixiv permanently rejects (`invalid_grant`) are evicted from rotation immediately; tokens are whitespace-trimmed on entry.

Open operations include health and the image proxy; system version and cached update status are open only in local mode, as are the dev docs (`/docs`, `/openapi.json`) — public mode answers them with 404 `not_found` so an anonymous instance neither advertises its build nor its endpoint contract. The auth/onboarding surface (login, logout, OAuth, connectivity probes) is open only in local mode. In local mode, browsing, settings, downloads, events, and update check/apply require the Pixiv session; in public mode the browsing reads and events pass through the pool gate instead, while settings, downloads, and update check/apply remain operator-only — the auth surface answers 404 `not_found`, making the instance purely anonymous with no admin entry. `GET /auth/status` stays open in both modes and carries `public_read`, which the frontend consumes to hide every login affordance (the `/login` and `/me` routes, the sidebar sign-in entry, and login-state mutation controls such as bookmark, follow, and batch/card download); local mode keeps them unchanged. In public mode that status is anonymous for every caller — `authenticated` is always false and identity fields are never reported, so a persisted operator session leaks nothing through the open endpoint. The update apply operation also requires `X-PixivBiu-App`, sent by the shared frontend API client; preserve this browser CSRF guard and the absence of cross-origin permission to send it. It is not a secret or a replacement for network access control.

Public deployments get a per-IP abuse guard on API reads: GET/HEAD requests under `/api/v1` (excluding `/health` and `/proxy/img`, which carry their own protections; SPA assets and docs are outside the API base) are budgeted 300/minute per client IP, answered past the budget with the structured 429 `rate_limited` envelope (`kind=app`, empty message, `Retry-After`). The middleware registers after httplog, so overflow still lands in the single request log. The client IP comes from chi RealIP's rewritten `RemoteAddr` — a deployment exposed beyond loopback should terminate at a reverse proxy that sets `X-Forwarded-For`, because a directly reachable core trusts any forwarded value and the guard's per-IP table is deliberately fail-open under spoofed-IP floods.
...
The codes are `unauthenticated` (401), `bad_request` (400), `forbidden` (403), `not_found` (404), `conflict` (409), `rate_limited` (429), `upstream_error` (502), and `internal_error` (500). `rate_limited` covers both the local per-IP read guard (`kind=app`, empty message) and an upstream Pixiv 429 (`kind=upstream` with the upstream block). An expired access token alone does not require logging in again: refresh and retry can preserve the session.

Middleware order is RequestID → RealIP → httplog → per-IP API read budget → generated routes. httplog owns panic recovery; adding another Recoverer loses the intended structured error path. Request errors and attributes attach to the single request log via `httplog.SetError/SetAttrs`. Background events use slog. Both use ECS normalization, such as `@timestamp`, `log.level`, and `error.message`. Backend logs stay in English.
