...
Reads have two modes. The default local mode serves read endpoints only to that shared session, exactly as before public reads existed. Setting `pixiv.public_read_enabled` switches to public-site mode: read endpoints (illustration/user/search/ranking and the SSE stream) pass a dedicated gate when `pixiv.service_refresh_tokens` holds at least one token, and each upstream read authenticates as a token drawn round-robin from that pool — never the operator session, with no fallback in either direction (a drained pool fails closed even if the operator is logged in; the validator refuses enabling the switch with an empty pool). While public mode is on, even the signed-in operator's reads upstream as pool identities: per-browser user tokens are explicitly deferred, so features that need the operator's own identity upstream (such as private bookmarks) are unavailable until that lands. Mutations never use the gate — bookmark, follow, server-side download jobs, config patches, and system actions stay operator-only. Pool tokens that Pixiv permanently rejects (`invalid_grant`) are evicted from rotation immediately; tokens are whitespace-trimmed on entry.

Open operations include health, the image proxy, system version, and cached update status; the auth/onboarding surface (login, logout, OAuth, connectivity probes) is open only in local mode. In local mode, browsing, settings, downloads, events, and update check/apply require the Pixiv session; in public mode the browsing reads and events pass through the pool gate instead, while settings, downloads, and update check/apply remain operator-only — the auth surface answers 404 `not_found`, making the instance purely anonymous with no admin entry. `GET /auth/status` stays open in both modes and carries `public_read`, which the frontend consumes to hide every login affordance (the `/login` and `/me` routes, the sidebar sign-in entry, and login-state mutation controls such as bookmark, follow, and batch/card download); local mode keeps them unchanged. The update apply operation also requires `X-PixivBiu-App`, sent by the shared frontend API client; preserve this browser CSRF guard and the absence of cross-origin permission to send it. It is not a secret or a replacement for network access control.
